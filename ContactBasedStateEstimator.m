function [r,xtraj,utraj,ltraj,ljltraj,z,F,info,prog] = ContactBasedStateEstimator(xtraj,utraj,ltraj,ljltraj)
...
y_filt = filtfilt(b, a, OptiData(:, 3)) - OptiData(end, 3) + 0.0353;
...
%LOLOLOL redo this with R = Rotx(ang)
%State = R*[x_filt, y_filt, 0];
%x = State(1) etc
Rot = [cos(Gdir), -sin(Gdir); sin(Gdir), cos(Gdir)];
for i = 1:length(x_filt)
    State = Rot*[x_filt(i); y_filt(i)];
    x_filt(i) = State(1);
    y_filt(i) = State(2);
    Vel = Rot*[xdot(i); ydot(i)];
    xdot(i) = Vel(1);
    ydot(i) = Vel(2);
    Acc = Rot*[xddot(i); yddot(i)];
    xddot(i) = Acc(1);
    yddot(i) = Acc(2);
...
for i=2:N %length(scale_sequence)
    ...
    for j = 2:i
        uIMU = x_sensor(j, 1:3)';
        t = times(inds(j)) - times(inds(j-1));
        IMU_fun = @(x, oldx) IMUcost(x, oldx, uIMU, t);
        IMUerr_cost = FunctionHandleObjective(2,IMU_fun);
        prog = prog.addCost(IMUerr_cost,{prog.x_inds(4:6, j); prog.x_inds(4:6, j-1)});
        
%        uGT = [x_gt(j, 1); x_gt(j, 2); x_gt(j, 3)];
%        GT_fun = @(x) GTcost(x, uGT);
%        GTerr_cost = FunctionHandleObjective(1, GT_fun);s
...
%% cost fun!
    function [f, df] = IMUcost(x, oldx, u, t)
        acc = [1/t*(x(1)-oldx(1)); 1/t*(x(2)-oldx(2)); x(3)];
