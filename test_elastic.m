...
[states, times] = elasticLCP(p, x0, tf);
...
% traj = xtraj_elastic.eval(xtraj_elastic.getBreaks());
% x = traj(1, :);
% z = traj(2, :);
% theta = traj(3, :);
% xdot = traj(4, :);
...
% dt = diff(times);
% sensor_inds = round(linspace(1, numel(xtraj_elastic.getBreaks()), 120*tf));
