function [r,xtraj,utraj,ltraj,z,F,info,prog] = contactBasedStateEstimator(times, x0, xddot, yddot, thetadot, G, scale)
...
% these are to fix uniqueness in x
x0min.base_x = x0(1);
x0min.base_z = 0;
...
%     [xtraj, utraj, ltraj, ~, z, F, info] = solveTraj(prog, tf, traj_init);
    ...
    prog = addStateConstraint(prog, BoundingBoxConstraint(double(x0min), double(x0max)), 1);
%    if scseq == 1
        traj_init.x = PPTrajectory(foh([0, tf/N],[x0, x0]));%[double(x0rand),double(x0rand)]));
%    else
%        traj_init.x = xtraj;
%        traj_init.l = ltraj;
%    end
...
%% cost fun!
    ...
    function [f, df] = THETAcost(x, oldx, u)
        costmat = [x-oldx];
        f = 10*pi/10*(costmat-u)^2;
        df = 10*pi/10*2*(costmat-u)*[1, -1];
    end
